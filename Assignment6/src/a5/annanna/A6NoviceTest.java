...
public class A6NoviceTest {
	...
	/*
	 * This test makes sure that the new getter and setter methods work with
	 * a coordinate in the parameters.
	 */
	...
	assertEquals("Pixel retrieved after setting does not match expected value", RED, p.getPixel(0, 1));
	// RED is expected
	assertEquals("Pixel retrieved after setting does not match expected value", BLUE, p.getPixel(2, 1));
	assertEquals("Pixel retrieved after setting doesn't match expected value", WHITE, p.getPixel(1, 2));
    }
	...
	assertEquals("The picture is supposed to have a next element", true, myIterator.hasNext());
	myIterator.next();
	myIterator.next();
	myIterator.next();
	myIterator.next();
	assertEquals("The picture isn't supposed to have a next element", false, myIterator.hasNext());
	...
	/*
	 * If the iterator ran in column major order instead, then the second
	 * and third tests would fail.
	 */
	PictureImpl q = new PictureImpl(2, 2);
	q.setPixel(0, 0, RED);
	q.setPixel(1, 0, RED);
	 ...
	 * calls the iterator() method on it, creating a subPicIterator object.
	 * It then goes through both iterators to make sure that the Pixels
	 * overlap correctly.
	 */
	Picture p = new PictureImpl(3, 3);
	p.setPixel(0, 0, RED);
	...
	assertEquals("The pixel at subpicture(0, 0) should match the pixel at picture(1, 1)", pictureIterator.next(),
		subPicIterator.next());
	...
	assertEquals("The next subPicture pixel should be the pixel at picture(1, 2)", subPicIterator.next(),
		p.getPixel(1, 2));
	assertEquals("The next subPicture pixel should be the pixel at picture(2, 2)", subPicIterator.next(),
		p.getPixel(2, 2));
    }
	...
	/*
	 * This method acts similarly to the one above, but it uses coordinates
	 * as parameters for the extract method.
	 */
	Picture p = new PictureImpl(3, 3);
	p.setPixel(0, 0, RED);
	p.setPixel(1, 0, RED);
