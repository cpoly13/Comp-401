...
public class A6AdeptTest {
    ...
    /*
     * tests to make sure that the sample iterator works as specified. Checks
     * this by comparing the call to next() and the expected values for all
     * pixel locations that the iterator should collect
     */
    @Test
	...
	for (int i = 0; i < 9; i++) {
	    assertEquals(
		    "Pixel retrieved by the iterator does not match the pixel" + "expected from the source picture",
		    RED, sampIter.next());
	}
    ...
    /*
     * specific testing of the exceptions expected when an invalid sample
     * iterator is constructed. Exceptions include negative dx and dy values,
     * along with initial x and y values not being in the source picture. It
     * also takes into account the fact that an IllegalArgumentException should
     * be generated, not a RuntimeException
     */
    @Test
    ...
    /*
     * specific testing of the exceptions expected when an invalid window
     * iterator is constructed. Exceptions include negative width and height
     * values, along with initial width and height values not being in the
     * source picture
     */
    @Test
