...
public class A6NoviceTest {
    ...
    /*
     * tests the extract method using coordinates by comparing the getPixel
     * value from the subpicture to the original location in the source picture
     */
    @Test
    ...
    /*
     * method to test whether the next and hasNext methods work correctly, also
     * checks for the expected exceptions in the next() method
     */
    @Test
	...
	/*
	 * loop to go through the whole picture where the call to hasNext should
	 * return true, and also should return a red value on the first and
	 * third lines, and green on the second and fourth
	 */
	for (int i = 0; i < 4; i++) {
	    for (int j = 0; j < 4; j++) {

		assertEquals("The hasNext method should return true , but it did not", true, iter1.hasNext());
		if (i % 2 == 0)
		    assertEquals("The hasNext method should return a RED pixel , but it did not", RED, iter1.next());
		else
		    assertEquals("The hasNext method should return a GREEN pixel , but it did not", GREEN,
			    iter1.next());
	    }
	}
