...
public class A6NoviceTest {
	...
	public void getSetPixelTest(){
		...
		Coordinate ayy = new Coordinate(2,0);
		Coordinate bee = new Coordinate(1,1);
		Coordinate cee = new Coordinate(0,0);
		Coordinate dee = new Coordinate(2,1);
		
		
		assertEquals("Result from getPixel does not match correct pixel",
				p.getPixel(0, 0), p.getPixel(cee));
		assertEquals("Result from getPixel does not match correct pixel",
				p.getPixel(2, 1), p.getPixel(dee));
		assertEquals("Result from getPixel does not match correct pixel",
				p.getPixel(0,2), p.getPixel(eff));
		assertEquals("Result from getPixel does not match correct pixel",
				p.getPixel(2, 2), p.getPixel(gee));
	}
	...
	public void testExtract(){
		...
		Coordinate dee = new Coordinate(3,2);
		
		SubPicture subby = p.extract(ayy, cee);
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(1, 1), subby.getPixel(0,0));
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(1, 2), subby.getPixel(0,1));
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(3, 2), subby.getPixel(2,1));
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(2, 3), subby.getPixel(1,2));
		
		SubPicture subster = p.extract(ayy, bee);
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(1, 2), subster.getPixel(0,1));
		assertEquals("Pixel retrieved from subpicture doesn't match og pic",
				p.getPixel(2, 2), subby.getPixel(1,1));
	}
	...
	public void testIterator(){
		...
		assertEquals("Pixel returned from next does not match",
				p.getPixel(0, 0), weirdo.next());
		assertEquals("Pixel returned from next does not match",
				p.getPixel(1, 0), weirdo.next());
		assertEquals("Pixel returned from next does not match",
				p.getPixel(0, 1), weirdo.next());
		assertEquals("Pixel returned from next does not match",
