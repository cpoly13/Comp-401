...
public class A6AdeptTest {
    ...
    @Test // tests that a sampleIterator of (0,0,1,1) is equivalent to a normal
	  // picture iterator
    ...
    @Test // tests that a sampleIterator Iterates properly with higher dx dy
	  // values
	...
	for (int y = 0; y <= 5 - windowHeight; y++) {
		...
		assertTrue("Window test extracts Incorrect windows",
			comparePictures(testPic.extract(x, y, windowWidth, windowHeight), window_iter.next()));
	    }

	}
    }

    @Test // tests that a tile call returns proper subPictures
	...
	for (int y = 0; y < testPic.getHeight() / tileHeight && tile_iter.hasNext(); y++) {
	    for (int x = 0; x < testPic.getWidth() / tileWidth && tile_iter.hasNext(); x++) {
		// ("Extracting " + x * tileWidth + y * tileHeight + tileWidth +
		// tileHeight);
		assertTrue("Tile test extracts Incorrect tiles", comparePictures(
			testPic.extract(x * tileWidth, y * tileWidth, tileWidth, tileHeight), tile_iter.next()));
	    }
	}
    }
