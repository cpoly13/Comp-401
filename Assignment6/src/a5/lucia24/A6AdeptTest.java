...
public class A6AdeptTest {
	...
	public void windowTest() {
		...
		assertEquals("", windowPics[6].getPixel(0, 0), 
				windowPics[8].getPixel(0, 0));
		assertEquals("", windowPics[9].getPixel(1, 1), 
				windowPics[13].getPixel(1, 0));
	...
	public void tileTest(){
		...
		assertEquals("", tilePics[0].getPixel(1, 1), 
				tilePics[1].getPixel(0, 1));
		assertEquals("", tilePics[2].getPixel(1,0), 
				tilePics[3].getPixel(0, 0));
	...
	public void exceptionTest(){
		...
		try{
			Iterator<SubPicture> tile_iter = p.tile(10, 10);
		...
		try{
			Iterator<SubPicture> window_iter = p.window(-1, -1);
			fail("shouldnt have done that bro");
		} catch (IllegalArgumentException e) {
		} catch (RuntimeException e) {
			fail ("nope");
		...
		try{
			Iterator<Pixel> sample_iter = p.sample(1, 1, 0, 0);
			fail("shouldnt have done that bro");
		} catch (IllegalArgumentException e) {
		} catch (RuntimeException e) {
			fail ("nope");
