...
public class A6NoviceTest {
	...
	public void newExtractTest() {
		...
		SubPicture sub2;
		try {
			sub2 = pic.extract(new Coordinate(-1, 0), new Coordinate(1, 1));
			fail("Expected extraction to fail");
		} catch (IllegalArgumentException e) {

		}
		try {
			sub2 = pic.extract(new Coordinate(0, -1), new Coordinate(1, 1));
			fail("Expected extraction to fail");
		} catch (IllegalArgumentException e) {

		}
		try {
			sub2 = pic.extract(new Coordinate(0, 0), new Coordinate(WIDTH, HEIGHT));
			fail("Expected extraction to fail");
		} catch (IllegalArgumentException e) {

		}
		try {
			sub2 = pic.extract(new Coordinate(0, 0), new Coordinate(WIDTH, 2));
			fail("Expected extraction to fail");
		} catch (IllegalArgumentException e) {

		}
		try {
			sub2 = pic.extract(new Coordinate(0, 0), new Coordinate(2, HEIGHT));
			fail("Expected extraction to fail");
		} catch (IllegalArgumentException e) {
