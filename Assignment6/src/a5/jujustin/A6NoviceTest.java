...
public class A6NoviceTest {
	...
	assertEquals("Result from extract equals subpicture", p.extract(c, c2).getHeight(), s.getHeight());
	assertEquals("Result from extract equals subpicture", p.extract(c, c2).getWidth(), s.getWidth());

	// checking extract to correct subpicture object
	for (int y = 0; y < c2.getY() - c.getY(); y++) {
	    for (int x = 0; x < c2.getX() - c.getX(); x++) {
		assertEquals("Result from extract doesn't equal subpicture", p.extract(c, c2).getPixel(x, y),
			s.getPixel(x, y));
	    }
	}
