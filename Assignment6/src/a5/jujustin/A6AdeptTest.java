...
public class A6AdeptTest {
	...
	try {
	    ...
	    for (int x = 0; x < 15; x++) {
		assertEquals("Iterator Pixel tests", pic1.next(), red);
	    }
	...
	for (int x = 0; x < 3; x++) {
	    for (int y = 0; y < 3; y++) {
		assertEquals("Iterator Pixel tests", pic.next(), p.getPixel(y, x));
		if (x != 2) {
		    assertEquals("In Bounds", pic.hasNext(), true);
		}
	    }
	}
	assertEquals("Out of Bounds check", pic.hasNext(), false);
