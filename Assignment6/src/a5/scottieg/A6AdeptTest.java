...
public class A6AdeptTest {
	...
	public void testSampleMethod(){
		...
		for(int i=0; i<5; i++){
			for(int j=0; j<5; j++){
				switch ((i+j)%5){
				case 0: p.setPixel(i, j, RED);
						break;
				case 1: p.setPixel(i, j, GREEN);
						break;
				case 2: p.setPixel(i, j, BLUE);
						break;
				case 3: p.setPixel(i, j, WHITE);
						break;
				case 4: p.setPixel(i, j, BLACK);
				        break;
				}
			}
		}
		...
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(2, 1));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(4, 1));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(0, 2));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(2, 2));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(4, 2));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(0, 3));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(2, 3));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(4, 3));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(0, 4));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(2, 4));
		assertEquals("Iterator returned wrong pixel",
				iter.next(),p.getPixel(4, 4));
			...
			for(int i=0; i<3; i++){
				for(int j=0; j<2; j++)
					assertEquals("Iterator returned wrong subPicture",
								current.getPixel(i, j), p.extract(s.getXOffset(), s.getYOffset(),
								s.getWidth(), s.getHeight()).getPixel(i, j));
			}
		}
