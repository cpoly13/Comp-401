...
public class A6NoviceTest {
	...
	assertEquals("Result from getX() does not match value given in constructor", 1, c.getX());
	assertEquals("Result from getY() does not match value given in constructor", 2, c.getY());
    }

    /*
     * @Test public void testCoordinateConstructorExceptions(){ try { Coordinate
     * c = new Coordinate(-1, 0);
     * fail("Expected IllegalArgumentException x < 0"); } catch
     * (IllegalArgumentException e) { } catch (RuntimeException e) {
     * fail("Expected IllegalArgumentException but got generic RuntimeException"
     * ); }
     * 
     * try { Coordinate c = new Coordinate(0, -1);
     * fail("Expected IllegalArgumentException y < 0"); } catch
     * (IllegalArgumentException e) { } catch (RuntimeException e) {
     * fail("Expected IllegalArgumentException but got generic RuntimeException"
     * ); } }
     */
	...
	assertEquals("Pixel retrieved does not equal what it was set to", p.getPixel(c1), RED);
	assertEquals("Pixel retrieved does not equal what it was set to", p.getPixel(c2), GREEN);
	assertEquals("Pixel retrieved does not equal what it was set to", p.getPixel(c3), BLUE);
	assertEquals("Pixel retrieved does not equal what it was set to", p.getPixel(c4), WHITE);
    }
