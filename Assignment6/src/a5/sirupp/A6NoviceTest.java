...
public class A6NoviceTest {
	...
	public void getSetTest() {
		// Creating a picture impl and coordinates to test the get Coordinate method
		...
		assertEquals("The returned pixel does not match its intended set value", 
				BLACK, newPicture.getPixel(testCoordinate));
		assertEquals("The returned pixel does not match its intended set value", 
				BLACK, newPicture.getPixel(testCoordinate2));
		assertEquals("The returned pixel does not match its intended set value", 
				WHITE, newPicture.getPixel(testCoordinate3));
		assertEquals("The return pixel does not match its intended set value", 
				WHITE, newPicture.getPixel(testCoordinate4));
		...
		// Checking if Coordinate setPixel method set (and get method returns) the correct pixels
		assertEquals("The provided set pixel method does not work", 
				RED, secondPicture.getPixel(0,0));
		assertEquals("The provided set pixel method does not work", 
				RED, secondPicture.getPixel(3,3));
		assertEquals("The provided set pixel method does not work", 
				GREEN, secondPicture.getPixel(2,2));
		assertEquals("The provided set pixel method does not work", 
				GREEN, secondPicture.getPixel(1,1));
	...
	public void extractTest() {
		...
		assertEquals("The subpicture's pixel doesn't match the big picture's", 
				BLUE, subPicture1.getPixel(0,0));
		assertEquals("The subpicture's pixel doesn't match the big picture's", 
				BLUE, subPicture1.getPixel(3,3));

		// Testing pixels between subpicture and source picture
		assertEquals("The subpicture's pixel doesn't match the big picture's", 
				GREEN, subPicture2.getPixel(0, 0));
		assertEquals("The subpicture's pixel doesn't match the big picture's", 
				GREEN, subPicture2.getPixel(4, 4));
	...
	public void nextTest() {
		...
		// Testing iterator's returned pixels and hasNext method reference within next
