...
public class A6NoviceTest {
	...
	assertEquals("x does not match value given to constructor", 3, c.getX());
	assertEquals("y does not match value given to constructor", 5, c.getY());
    }
	...
	assertEquals("getPixel overload methed did not return expected Pixel", BLACK, p.getPixel(black));

	Coordinate white = new Coordinate(4, 2);
	p.setPixel(white, WHITE);

	assertEquals("setPixel overload method did not cause picture to return expected Pixel", WHITE,
		p.getPixel(4, 2));

	Coordinate topLeftCorner = new Coordinate(2, 1);
	Coordinate bottomRightCorner = new Coordinate(5, 3);
	Picture subP1 = p.extract(topLeftCorner, bottomRightCorner);

	assertEquals("The SubPicture's getPixel method did not return expected pixel", GRAY, subP1.getPixel(2, 2));
	assertEquals("The SubPicture's getPixel method did not return expected pixel", WHITE, subP1.getPixel(2, 1));
	assertEquals("The SubPicture's getPixel method did not return expected pixel", BLACK, subP1.getPixel(0, 2));
    }
	...
	assertEquals("iterator did not return expected Pixel", GRAY, iter.next());
	for (int i = 0; i < 3; i++) {
	    iter.next();
	}
