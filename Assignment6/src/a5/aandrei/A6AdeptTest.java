...
public class A6AdeptTest {
  ...
  public void testWindowIterator() {
    ...
    assertEquals("iterator did not return expected SubPicture",
        BLACK, iter.next().getPixel(1, 0));
    iter.next();
    assertEquals("iterator did not return expected SubPicture",
        WHITE, iter.next().getPixel(0, 1));
    for (int i = 0; i < 3; i++) {
      iter.next();
    }
    assertEquals("iterator did not return expected SubPicture",
        WHITE, iter.next().getPixel(0, 0));
    iter.next();
    assertEquals("iterator did not return expected SubPicture",
        WHITE, iter.next().getPixel(1, 2));
    iter.next();
    assertEquals("iterator did not return expected SubPicture",
        BLACK, iter.next().getPixel(0, 1));
    iter.next();
    try {
      iter.next();
      fail("Expected NoSuchElementException");
  ...
  public void testTileIterator() {
    ...
    assertEquals("iterator did not return expected SubPicture",
        BLACK, iter.next().getPixel(2, 2));
    iter.next();
    assertEquals("iterator did not return expected SubPicture",
        WHITE, iter.next().getPixel(1, 1));
    assertEquals("iterator did not return expected SubPicture",
        WHITE, iter.next().getPixel(1, 2));
    iter.next();
    assertEquals("iterator did not return expected SubPicture",
        BLACK, iter.next().getPixel(2, 3));
    try {
      iter.next();
      fail("Expected NoSuchElementException");
    } catch (NoSuchElementException e) {
