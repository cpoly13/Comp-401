...
public class A6AdeptTest {
	...
	/*
	 * Test of the sample method in adept,
	 * by getting the pixels manually from coordinates
	 * and comparing them to the iterator
	 */
	@Test
	public void testSample() {
		...
		assertEquals("Sample iterator does not match expected value",
				testing1, test1);
		assertEquals("Sample iterator does not match expected value",
				testing2, test2);
		assertEquals("Sample iterator does not match expected value",
		...
		assertEquals("Sample iterator does not match source picture",
				testing2nd1, testing3rd1);
		assertEquals("Sample iterator does not match source picture",
				testing2nd2, testing3rd2);
		assertEquals("Sample iterator does not match source picture",
				testing2nd3, testing3rd3);
		assertEquals("Sample iterator does not match source picture",
				testing2nd4, testing3rd4);
		try {
			Picture ptest = new PictureImpl(21, 19);
			Iterator<Pixel> sample_itertest = ptest.sample(2, 7, 2, 9);
			sample_iter.remove();
		...
		try {
			...
			Iterator<Pixel> sample_itertest2 = ptest2.sample(-6, -8, -2, -7);
			fail("Illegal argument exception expected for sample parameters being negative or not fitting within picture"); }
			catch(IllegalArgumentException e) {
			} catch (RuntimeException e) {fail("Expected IllegalArgumentException but got generic RuntimeException");}
		
		try {
			Picture ptest2 = new PictureImpl(6,4);
			Iterator<Pixel> sample_itertest2 = ptest2.sample(8, 8, 1, 2);
			fail("Illegal argument exception expected for sample parameters being negative or not fitting within picture"); }
			catch(IllegalArgumentException e) {
			} catch (RuntimeException e) {fail("Expected IllegalArgumentException but got generic RuntimeException");}
	 ...
	 * from a picture and them comparing them to the window iterator
	 * which slides a window by the given dimensions across a parent picture.
	 * They're compared by looping through each window and it's corresponding
	 * solution and ensuring that they have identical pixels and dimensions.
	 */
	@Test
	public void testWindow() {
		...
		if ((p1.getWidth() != t1.getWidth()) || (p1.getHeight() != t1.getHeight()) ||
				(p2.getWidth() != t2.getWidth()) || ((p2.getHeight() != t2.getHeight()) ||
				(p3.getWidth() != t3.getWidth()) || (p3.getHeight() != t3.getHeight()) ||
				(p4.getWidth() != t4.getWidth()) || (p4.getHeight() != t4.getHeight())) ||
				...
				(p10.getWidth() != t10.getWidth()) || ((p10.getHeight() != t10.getHeight()) ||
				(p11.getWidth() != t11.getWidth()) || (p11.getHeight() != t11.getHeight()) ||
				(p12.getWidth() != t12.getWidth()) || (p12.getHeight() != t12.getHeight()))) {
					b = false;
				}
		assertEquals("window() does not return the expected subpicture",
				b, b1);
		
		
		try {
			Picture ex = new PictureImpl(6,4);
			Iterator<SubPicture> window_itertest = ex.window(7, 3);
	 ...
	 * given dimensions. I find those tiles manually and then compare them 
	 * to the iterator. They're compared by taking each tile and it's corresponding
	...
	public void testTile() {
		...
		assertEquals("tile() does not return the expected subpicture",
				b,b1);
		
		//testing for the correct exceptions
		try {
			Picture ex = new PictureImpl(6,4);
			Iterator<SubPicture> tile_itertest = ex.tile(7, 3);
