...
public class A6AdeptTest {
	...
	assertEquals("Window does not slide properly", source.extract(0, 0, 3, 2).getWidth(), window.next().getWidth());
    }
	...
	assertEquals("Tiles are not placed correctly", source.extract(0, 0, 2, 2).getHeight(), tile.next().getHeight());
	assertEquals("Tiles are not placed correctly", source.extract(0, 0, 2, 2).getWidth(), tile.next().getWidth());
    }
}
