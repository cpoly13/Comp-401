...
public class A6NoviceTest {
    ...
    // coordinate is comparing the coordinate passed through with the actual
	...
	assertEquals("Pixel at SubPicture does not match accordingly to Picture", source.getPixel(2, 1).getIntensity(),
		sp.getPixel(0, 0).getIntensity(), 0.01);
	assertEquals("Pixel at SubPicture does not match accordingly to Picture", source.getPixel(3, 3).getIntensity(),
		sp.getPixel(1, 2).getIntensity(), 0.01);

	Picture source1 = new PictureImpl(10, 11);
	SubPicture sp1 = new SubPictureImpl(source, 3, 0, 3, 4);

	assertEquals("Pixel at SubPicture does not match accordingly to Picture", source1.getPixel(4, 1).getIntensity(),
		sp1.getPixel(1, 1).getIntensity(), 0.01);
	assertEquals("Pixel at SubPicture does not match accordingly to Picture", source1.getPixel(4, 2).getIntensity(),
		sp1.getPixel(1, 2).getIntensity(), 0.01);
	...
	assertEquals("x and y are not within range of width and height", source.getPixel(0, 0).getIntensity(),
		p.next().getIntensity(), 0.01);

	Picture source1 = new PictureImpl(6, 12);
	Iterator<Pixel> p1 = source.iterator();

	assertEquals("x and y are not within range of width and height", source1.getPixel(5, 0).getIntensity(),
		p1.next().getIntensity(), 0.01);
    }
