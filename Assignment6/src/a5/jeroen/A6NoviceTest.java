...
public class A6NoviceTest {
	...
	assertEquals("Coordinate does not store x correctly", c1.getX(), 2);
	assertEquals("Coordinate does not store y correctly", c1.getY(), 2);

	Picture p1 = new PictureImpl(6, 6);
	SubPicture sp1 = p1.extract(c1, c2);
	SubPicture sp2 = p1.extract(2, 2, 3, 3);

	assertEquals("SubPicture extract using Coordinates incorrect", sp1.getWidth(), 3);

	assertEquals("SubPicture extract using Coordinates incorrect", isTheSamePicture(sp1, sp2), true);
	...
	assertEquals("Iterator returned wrong pixel", iter.next(), p3.getPixel(1, 0));

	for (int x = 0; x < 8; x++) {
	    iter.next();
	}

	assertEquals("Iterator hasNext broken", iter.hasNext(), false);
    }
