...
public class A6AdeptTest {
	...
	for (int j = 0; j < 4; j++) {
		...
		for (int l = 0; l < p1.extract(i, j, 3, 2).getHeight(); l++) {
			...
			assertEquals(p1.extract(i, j, 3, 2).getPixel(k, l), windowPic.getPixel(k, l));
		    }
		}
	    }
	}
	...
	Picture p1 = genPicture(5, 5);
	Iterator<SubPicture> tileIter = p1.tile(2, 2);
	assertEquals(p1.extract(0, 0, 2, 2).getPixel(0, 0), tileIter.next().getPixel(0, 0));
	assertEquals(p1.extract(2, 0, 2, 2).getPixel(0, 0), tileIter.next().getPixel(0, 0));
	assertEquals(p1.extract(0, 2, 2, 2).getPixel(0, 0), tileIter.next().getPixel(0, 0));
	assertEquals(p1.extract(2, 2, 2, 2).getPixel(0, 0), tileIter.next().getPixel(0, 0));
    }
