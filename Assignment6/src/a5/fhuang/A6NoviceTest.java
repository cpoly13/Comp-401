...
public class A6NoviceTest {
	...
	assertEquals("Result from getX() does not match coordinate", 6, a.getX());
	assertEquals("Result from getY() does not match coordinate", 4, a.getY());
    }
	...
	assertEquals("Result from setPixel does not match coordinate", p1, p.getPixel(a));
	...
	assertEquals("Result from extract does not match subpicture", p.getPixel(b), result1.getPixel(b));
	assertEquals("Result from extract does not match subpicture", p.getPixel(c), result2.getPixel(a));
	...
	assertEquals("Result from iterator does not match picture", p1, iter.next());
