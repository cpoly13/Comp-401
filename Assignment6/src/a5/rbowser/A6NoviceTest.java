...
public class A6NoviceTest {
	...
	assertEquals("Result from getX() does not match source picture", 4, expected.getX());
	assertEquals("Result from getY() does not match source picture", 4, expected.getY());
    }
	...
	assertEquals("Expected pixel does not match the pixel from the subpicture", p.getPixel(1, 1),
		sub.getPixel(test));
	assertEquals("Expected pixel does not match the pixel from the subpicture", p.getPixel(2, 2),
		sub.getPixel(new Coordinate(1, 1)));
	assertEquals("Expected pixel does not match the pixel from the subpicture", p.getPixel(1, 2),
		sub.getPixel(new Coordinate(0, 1)));
	assertEquals("Expected pixel does not match the pixel from the subpicture", p.getPixel(2, 1),
		sub.getPixel(new Coordinate(1, 0)));

	p.setPixel(new Coordinate(1, 0), RED);
	assertEquals("Expected pixel does not match the pixel from the subpicture", RED, p.getPixel(1, 0));
