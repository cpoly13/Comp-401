...
public class A6AdeptTest {
    ...
    /*
     * testSample Tests Sample iterator of a Picture; Whether it returns correct
     * boolean hasNext() at the start and the end Tests if correct Pixels were
     * returned when next() is called Tests 2 Pictures, one 15 x 10 that has a
     * dx and dy > 1 The other calling sample(0,0,1,1), tests if same as
     * iterator in Novice
     */
    @Test
	...
	assertEquals("hasNext() at start is false", true, sample_iter.hasNext());
	assertEquals("First pixel in iterator is not pixel (2,3)", p.getPixel(2, 3), sample_iter.next());
	assertEquals("First pixel in iterator is not pixel (5,3)", p.getPixel(5, 3), sample_iter.next());
	assertEquals("Second pixel in iterator is not pixel (8,3)", p.getPixel(8, 3), sample_iter.next());
	assertEquals("Third pixel in iterator is not pixel (11,3)", p.getPixel(11, 3), sample_iter.next());
	assertEquals("Fourth pixel in iterator is not pixel (14,3)", p.getPixel(14, 3), sample_iter.next());
	assertEquals("Fifth pixel in iterator is not pixel (2,7)", p.getPixel(2, 7), sample_iter.next());
	assertEquals("Sixth pixel in iterator is not pixel (5,7)", p.getPixel(5, 7), sample_iter.next());
	assertEquals("Seventh pixel in iterator is not pixel (8,7)", p.getPixel(8, 7), sample_iter.next());
	assertEquals("Eight pixel in iterator is not pixel (11,7)", p.getPixel(11, 7), sample_iter.next());
	assertEquals("Ninth pixel in iterator is not pixel (14,7)", p.getPixel(14, 7), sample_iter.next());
	assertEquals("hasNext() at end is true", false, sample_iter.hasNext());
    ...
    /*
     * testWindow Tests 5 x 5 picture with a window iterator with width of 3 and
     * height of 2 Whether it returns correct boolean hasNext() at the start and
     * the end Tests if correct SubArea were returned when next() is called
     */
    @Test
    public void testWindow() {
	Picture p = new PictureImpl(5, 5);
	Iterator<SubPicture> window_iter = p.window(3, 2);
	assertEquals("hasNext() at start is false", true, window_iter.hasNext());
	...
	assertEquals("hasNext() at end is true", false, window_iter.hasNext());
    }

    /*
     * testTile Tests 5 x 5 picture with a tile iterator with width of 2 and
     * height of 2 Whether it returns correct boolean hasNext() at the start and
     * the end Tests if correct SubArea were returned when next() is called
     */
    @Test
	...
	assertEquals("hasNext() at start is false", true, tile_iter.hasNext());
	...
	assertEquals("hasNext() at end is true", false, tile_iter.hasNext());

    }

    /*
     * compareSubPictures Static Method that takes in 2 SubPictures returns true
     * if their source, x and y offsets, width, and height are identical
     */
    public static boolean compareSubPictures(SubPicture a, SubPicture b) {
	return a.getSource().equals(b.getSource()) && (a.getHeight() == b.getHeight()) && (a.getWidth() == b.getWidth())
		&& (a.getXOffset() == b.getXOffset()) && (a.getYOffset() == b.getYOffset());
    }
}
