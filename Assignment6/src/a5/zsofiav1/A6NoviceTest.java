...
public class A6NoviceTest {
	...
	static public String[] getTestNames() {
		return new String[]{"testCoordinateSubPicture","testCoordinateGettersAndSetters", 
							"testIterator", "testExceptions"};
	}

	/* testCoordinateSubPicture
	 * Tests to see if a SubPicture extracted by giving 2 Coordinates 
	 * is identical to a SubPicture extracted by giving offsets and height/width
	 */
	@Test
	public void testCoordinatesSubPicture() {
		...
		assertEquals("Height does not match value given to constructor",
				5, sp.getHeight());	
		assertEquals("Source for Subpictures do not match",
				sp2.getSource(), sp.getSource());
		assertEquals("X offset for Subpictures do not match",
				sp2.getXOffset(), sp.getXOffset());
		assertEquals("Y offset for Subpictures do not match",
				sp2.getYOffset(), sp.getYOffset());
		assertEquals("Width for Subpictures do not match",
				sp2.getWidth(), sp.getWidth());
		assertEquals("Height for Subpictures do not match",
				sp2.getHeight(), sp.getHeight());	
	}
	/* testCoordinateGettersAndSetters
	 * Tests to see whether setting a Pixel using a Coordinate sets correct pixel
	 * Tests getter methods using Coordinates in both Pictures and SubPictures
	 */
	@Test
	public void testCoordinateGettersAndSetters() {
		...
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(e), sp.getPixel(a));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(f), sp.getPixel(b));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(h), sp.getPixel(d));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(i), sp.getPixel(e));
		...
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(i), sp2.getPixel(a));

	}
	/* testIterator
	 * Tests iterator of a Picture;
	...
	/* testExceptions
	 * Tests whether exceptions are correctly thown;
	 * 		- when a null picture tries to create an iterator
	 * 		- NoSuchElementException thrown when .next() is called after iterator is done
	 * 		- IllegalArgumentExceptions thrown when null coordinates are used for getPixel, setPixel, or extract
	 */
	@Test
