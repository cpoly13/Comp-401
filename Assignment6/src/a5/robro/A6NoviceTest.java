...
public class A6NoviceTest {
	...
	public void coordinateConstructorTest() {
		...
		assertEquals("X value does not match value given to constructor",
				3, c.getX());
		assertEquals("Y value does not match value given to constructor",
				1, c.getY());
	}
	...
	public void subPictureFromCoordinateTest() {
		...
		assertEquals("Subpicture source does not match expected source picture",
				p, sp.getSource());
		assertEquals("Subpicture width does not match expected value",
				3, sp.getWidth());
		assertEquals("Subpicture height does not match expected value",
				3, sp.getHeight());
		assertEquals("Subpicture xoffset does not match expected value",
				2, sp.getXOffset());
		assertEquals("Subpicture yoffset does not match expected value",
				0, sp.getYOffset());
	}
	...
	public void setGetPixelTest() {
		...
		p.setPixel(c, RED);
		assertEquals("Picture does not return expected pixel value",
				RED, p.getPixel(c));
		SubPicture sp = p.extract(c, c2);
		assertEquals("Subpicture does not return expected pixel value",
				p.getPixel(c), sp.getPixel(0,0));
	...
	public void iteratorTest() {
		...
		assertEquals("Iterator hasNext not functioning",
				true, iter.hasNext());
		assertEquals("Iterator returning incorrect pixel",
