...
public class A6NoviceTest {
	...
	public void testExtract (){
		...
		assertEquals("Pixels retrieved from subpicture does not match expected pixel value from source", p.getPixel(c5),sp.getPixel(c1));
		assertEquals("Pixels retrieved from subpicture does not match expected pixel value from source", p.getPixel(c6),sp.getPixel(c2));
		assertEquals("Pixels retrieved from subpicture does not match expected pixel value from source", p.getPixel(c8),sp.getPixel(c4));
		assertEquals("Pixels retrieved from subpicture does not match expected pixel value from source", p.getPixel(c9),sp.getPixel(c5));

		SubPicture sp2 = sp.extract(1,1, 1, 1);
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source", p.getPixel(c9), sp2.getPixel(c1));
