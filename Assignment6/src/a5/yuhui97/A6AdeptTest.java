...
public class A6AdeptTest {
	...
	public void testSampleIterator(){
		...
		assertEquals("Iterator does not retrieve correct sample", GREEN,
				sourceSampleIterator.next());
		
		assertEquals("Iterator does not retrieve correct sample", BLUE,
				sourceSampleIterator.next());
	...
	public void testWindowIterator(){
		...
		assertEquals("Iterator does not retrieve correct window", 
				source.getPixel(0, 0), subpic1.getPixel(0, 0));
		assertEquals("Iterator does not retrieve correct window", 
				source.getPixel(1, 0), subpic1.getPixel(1, 0));
		assertEquals("Iterator does not retrieve correct window",
				source.getPixel(0, 1), subpic1.getPixel(0, 1));
		assertEquals("Iterator does not retrieve correct window", 
				source.getPixel(1, 1), subpic1.getPixel(1, 1));
	}
}
