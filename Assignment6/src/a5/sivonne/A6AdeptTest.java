...
public class A6AdeptTest {
  ...
  public void TileTest() {
    ...
    assertEquals("Pixel value incorrect", tile_iter.next().getXOffset(),
        (p.extract(0, 0, 2, 2)).getXOffset());
    assertEquals("Pixel value incorrect", tile_iter.next().getXOffset(),
        (p.extract(2, 0, 2, 2)).getXOffset());
    assertEquals("Pixel value incorrect", tile_iter.next().getXOffset(),
        (p.extract(0, 2, 2, 2)).getXOffset());
    assertEquals("Pixel value incorrect", tile_iter.next().getXOffset(),
        (p.extract(2, 2, 2, 2)).getXOffset());
