...
public class A6NoviceTest {
	...
	private static final Coordinate CORNER_A = new Coordinate(1, 4);
	private static final Coordinate CORNER_B = new Coordinate(4, 1);
	...
	public void testSetGetPixel() {
		...
		assertEquals("getPixel does not match setPixel in Picture", 
				TEST_PIXEL, 
				TEST_PICTURE.getPixel(TEST_COORDS));
		assertEquals("setPixel set incorrect Pixel(s)", 
				TEST_PICTURE.getPixel(STD_COORDS_1),
				TEST_PICTURE.getPixel(STD_COORDS_2));
	}
	...
	public void testCoordBasedExtract() {
		...
		assertEquals("extract does not produce appropriate SubPicture based on Coords", 
				p.getPixel(1, 1), TEST_PICTURE.getPixel(testCoord));
	...
	public void testCoordSetPixel() {
		...
		assertEquals("setPixel does not produce same result as integer version",
				P_1.getPixel(TEST_COORDS), P_2.getPixel(TEST_COORDS));
	...
	public void testCoordGetPixel() {
		...
		assertEquals("getPixel does not produce same result as integer version",
				P_1.getPixel(TEST_COORDS), P_1.getPixel(4, 3));
	}
