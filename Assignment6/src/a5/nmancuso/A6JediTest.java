...
public class A6JediTest {
	...
	public void testExtractAndCoordinates(){
		...
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(d), sp.getPixel(a));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(f), sp.getPixel(b));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(e), sp.getPixel(c));
		assertEquals("Pixel retrieved from subpicture does not match expected pixel value from source",
				p.getPixel(g), sp.getPixel(d));
	}
	...
	public void zigZagTest() {
		...
		Coordinate b = new Coordinate(1,0);
		Coordinate c = new Coordinate(0,1);
		Coordinate d = new Coordinate(0,2);
		...
		//create iterator to test zigzag method
		...
		assertEquals("Third Pixel not iterated", GREEN, pix);
		
		//go to fourth iteration and test equivalence
		if(iter.hasNext()){
			pix = iter.next();
		}
		assertEquals("Fourth Pixel not iterated", BLACK, pix);
		
		//go to last iteration and test equivalence
		while(iter.hasNext()){
			pix=iter.next();
		}
		assertEquals("Last pixel not iterated", WHITE, pix);
	}
}
