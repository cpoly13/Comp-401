...
public class A6NoviceTest {
	...
	a.setPixel(Y, BLUE);
	a.setPixel(Z, GREEN);
	assertEquals("Pixel on coordinate X from picture a is different with the pixel setted", RED, a.getPixel(X));
	assertEquals("Pixel on coordinate Y from picture a is different with the pixel setted", BLUE, a.getPixel(Y));
	assertEquals("Pixel on coordinate Z from picture a is different with the pixel setted", GREEN, a.getPixel(Z));
    }
	...
	assertEquals("1st pixel get by the iterator is wrong", RED, it.next());
	assertEquals("2nd pixel get by the iterator is wrong", GREEN, it.next());
	assertEquals("3rd pixel get by the iterator is wrong", BLUE, it.next());
	assertEquals("4th pixel get by the iterator is wrong", RED, it.next());
	assertEquals("5th pixel get by the iterator is wrong", BLUE, it.next());
	assertEquals("6th pixel get by the iterator is wrong", GREEN, it.next());
    }
	...
	Iterator<Pixel> it = b.iterator();
	assertTrue("iterator unexpected end", it.hasNext());
	it.next();
	assertTrue("iterator unexpected end", it.hasNext());
	it.next();
	assertTrue("iterator unexpected end", it.hasNext());
	...
	assertTrue("iterator unexpected end", it.hasNext());
	it.next();
	assertFalse("iterator unexpected Pixel", it.hasNext());
    }
