...
public class A6AdeptTest {
	 ...
	 * Assumes the integer getters and setters work properly, and tests the overloaded version against those.
	 */
	...
	/*
	 * Test the overloaded version of extract.
	 * Assumes the "original" extract works properly and tests the overloaded version against it.
	 */
	@Test
	...
	/*
	 * Test the iterator for both a picture and a subpicture.
	 * Uses an array of coordinates - the order in which coordinates are set in the array corresponds to row major order.
	 * Also tests for null pictures trying to use the iterator.
	 */
	@Test
	public void testRowMajorPixelIterator(){
		...
		try{
		Picture not_p = null;
		Iterator<Pixel> not_p_iter = not_p.iterator();
		...
		while (sub_iter.hasNext()) {
			assertEquals("Iterator for pixel at " + sub_coords[j].getX() + " " + sub_coords[j].getY() + " does not match.",
