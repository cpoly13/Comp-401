...
public class A6NoviceTest {
	...
	p.setPixel(s, white);
	assertEquals("Result from getX() does not match proper Coordinate", s.getX(), 0);
	assertEquals("Result from getY() does not match proper Coordinate", s.getY(), 1);
    }
	...
	s.setPixel(x, white);
	assertEquals("Result from overload setters and getters does not match old method results", p.getPixel(0, 0),
		s.getPixel(x));
    }
	...
	while (iter.hasNext()) {
		...
		for (int j = 0; j < 3; j++) {
		    x = j;
		    test = iter.next();
		    assertEquals("Test intenity of final pixel does not match expected", test, p.getPixel(x, y));
		}
	    }
	}
