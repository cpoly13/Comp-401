...
public class A6AdeptTest {
	...
	@Test
	/**
	 * Ensures the sample method chooses the correct coordinates and retrieves the
	...
	public void sampleTest() 
	{
		...
		while (sampleIter.hasNext())
		{
			...
			assertEquals("Sample iterator doesn't have the correct coordinates,",
					nextSampleCoord.equals(white), true);
		}
	}
		...
		for (int i = 0; i + windowHeight <= pic.getHeight(); i++)
		{
			for (int j = 0; j + windowWidth <= pic.getWidth(); j++)
			{
				try
				{
					...
					Coordinate origin = new Coordinate(nextWindow.getXOffset(), 
							nextWindow.getYOffset());
		...
		for (int i = 0; i <= pic.getHeight() - tileHeight; i += tileHeight)
		{
			for (int j = 0; j <= pic.getWidth() - tileWidth; j += tileWidth)
			{
				try 
				{
					...
					Coordinate origin = new Coordinate(nextTile.getXOffset(), 
							nextTile.getYOffset());
					assertEquals("X coordinate of the window's origin is wrong,", origin.getX(), j);
					assertEquals("Y coordinate of the window's origin is wrong,", origin.getY(), i);
