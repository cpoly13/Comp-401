...
public class A6NoviceTest {
	...
	/*
	 * Tests whether the pixel getters and setters
	 * with coordinate inputs are correct
	 */
	@Test
	public void testCoordinateGettersAndSetters(){
		...
		assertEquals("Retrieved pixel does not match expected value.",
				RED, p.getPixel(C00));
		assertEquals("Retrieved pixel does not match expected value.",
				GREEN, p.getPixel(C01));
		assertEquals("Retrieved pixel does not match expected value.",
				BLUE, p.getPixel(C02));
		assertEquals("Retrieved pixel does not match expected value.",
				WHITE, p.getPixel(C10));
		assertEquals("Retrieved pixel does not match expected value.",
				BLACK, p.getPixel(C11));
		assertEquals("Retrieved pixel does not match expected value.",
				RED, p.getPixel(C12));
		assertEquals("Retrieved pixel does not match expected value.",
				GREEN, p.getPixel(C20));
		assertEquals("Retrieved pixel does not match expected value.",
				BLUE, p.getPixel(C21));
		assertEquals("Retrieved pixel does not match expected value.",
				WHITE, p.getPixel(C22));
	}

	/*
	 * Tests the extract overridden extract class that takes
	 * coordinate inputs. Also tests whether the order of
	 * the coordinates entered as paramaters matters.
	 */
	@Test
	public void testExtractWithCoordinates(){
		...
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C10), sp.getPixel(C00));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C11), sp.getPixel(C01));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C12), sp.getPixel(C02));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C20), sp.getPixel(C10));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C21), sp.getPixel(C11));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C22), sp.getPixel(C12));
		

		SubPicture sp2 = p.extract(C11, C10);
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C10), sp2.getPixel(C00));
		assertEquals("Pixel retrieved from subpicture does not match pixel from source.",
				p.getPixel(C11), sp2.getPixel(C01));
		
		try{
			SubPicture wrongSp = p.extract(new Coordinate(-1, -1), C01);
	...
	public void testSimpleIterator(){
		...
		for(int j=0; j<pa.length; j++){
			assertEquals("Iterator does not return has next",
					true, i.hasNext());
			assertEquals("Iterator does not return correct pixel", 
					pa[j], i.next());
		}
