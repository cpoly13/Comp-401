...
public class A6JediTest {
	...
	Pixel[] pa = { RED, GREEN, RED, RED, GREEN, BLUE, RED, BLUE, GREEN, RED, GREEN, BLUE, RED, GREEN, BLUE, GREEN,
		RED, BLUE, RED, GREEN, BLUE, BLUE, GREEN, BLUE };
	for (int i = 0; i < 24; i++) {
	    assertEquals("Iterator does not return true for hasNext()", true, zz.hasNext());
	    assertEquals("Pixel returned from iterator does not match expected Pixel", pa[i], zz.next());
	}
	assertEquals("Iterator does not return false for has next when the iterator is at end bounds", false,
		zz.hasNext());
